  ...
  <v-container class="mb-6">
    ...
    <v-card-text>
      ...
      <v-row justify="end">
        ...
        <run-only-text v-if="optionType == 'onlyText'"></run-only-text>
        <run-only-image v-if="optionType == 'onlyImage'"></run-only-image>
        <run-only-table v-if="optionType == 'onlyTable'"></run-only-table>
      </v-row>
      <v-row v-if="optionType == 'onlyText'">
        <v-col cols="12">
          <v-simple-table>
      ...
      <v-row v-else-if="optionType == 'onlyTable'">
        <v-col cols="12">
          <run-only-table-list></run-only-table-list>
        </v-col>
      </v-row>
    </v-card-text>
  </v-container>
...
import RunOnlyImage from "@/components/run/question/options/OnlyImage.vue"
import RunOnlyText from "@/components/run/question/options/OnlyText.vue"
import RunOnlyTable from "@/components/run/question/options/OptionTable.vue"
import {Component, Vue} from 'vue-property-decorator'
import {getModule} from 'vuex-module-decorators'
import {OptionModule} from '@/store/modules/OptionModule'
import Option from '@/models/Option'
import RunOnlyTableList from "@/components/run/question/options/lists/OnlyTableList.vue"

@Component({
  components: {RunOnlyText, RunOnlyImage, RunOnlyTable, RunOnlyTableList}
})
...
export default class Options extends Vue {
  ...
  addOption() {
    ...
    this.optionStore.openDialog(true)
    this.optionStore.setDialogOptionTableType("COL")
  }
